...
// Short day/month badge for card tiles, e.g. "7/8" for August 7th. Day
// defaults to 1 for month/year-only releases (see legacyMonthYearToIso),
// so this reads as "1/8" for those — consistent with that convention,
// not a bug.
...
export function recordToRelease(record) {
  ...
  return {
    ...
    coverOverride: f['Cover Override'] || '',
    addedBy: f['Added By'] || '',
...
export function releaseToFields(entry) {
  const fields = {
    'Artist': entry.artist || '',
    'Title': entry.title || '',
    'Genre Family': entry.genreFamily || 'Ambient',
    'Genre': entry.genre || [],
  ...
  if (entry.spotifyUrl) fields['Spotify URL'] = entry.spotifyUrl;
  if (entry.bandcampUrl) fields['Bandcamp URL'] = entry.bandcampUrl;
  if (entry.coverOverride) fields['Cover Override'] = entry.coverOverride;
  if (entry.addedBy) fields['Added By'] = entry.addedBy;
...
// ---- Spotify embed ----
// Accepts whatever a contributor pastes — a full open.spotify.com link
// (with or without a ?si= tracking param) or a spotify:album:ID URI — and
// normalizes it to an embeddable player URL. Returns null for anything
// unrecognized so the caller can skip rendering a player rather than
...
// Contributor-pasted URLs rendered as an href (e.g. Bandcamp link-outs)
// must be scheme-checked, not just esc()'d — esc() stops attribute
// breakout but a `javascript:` URL is still a valid, dangerous href.
// Returns the URL only if it's plain http(s); null otherwise. Unlike
// Spotify (embed URL is rebuilt from a regex-extracted ID), Bandcamp
// links pass through as-is because labels often use custom domains —
// so the scheme check is the whole defense.
