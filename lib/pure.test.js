...
test('formatShortDate: day/month badge, "?" for unparseable input', () => {
  assert.equal(formatShortDate('2026-08-07'), '7/8');
  assert.equal(formatShortDate('2026-01-01'), '1/1');
...
test('recordToRelease / releaseToFields round-trip the core fields', () => {
  const record = {
    ...
    fields: {
      ...
      'Bandcamp URL': 'https://zake.bandcamp.com/album/hh',
      'Cover Override': 'https://example.com/cover.jpg',
      'Added By': 'Stephen',
      'Added At': 1000,
      'Updated At': 2000,
    },
  ...
  assert.equal(release.genreFamily, 'Ambient');
  ...
  assert.equal(release.bandcampUrl, 'https://zake.bandcamp.com/album/hh');
  assert.equal(release.coverOverride, 'https://example.com/cover.jpg');
  assert.equal(release.addedBy, 'Stephen');

  const fields = releaseToFields(release);
  assert.equal(fields['Artist'], 'zakè');
  assert.equal(fields['Genre Family'], 'Ambient');
  ...
  assert.equal(fields['Bandcamp URL'], 'https://zake.bandcamp.com/album/hh');
...
test('recordToRelease: defaults missing fields instead of crashing', () => {
  ...
  assert.equal(release.genreFamily, '');
});

test('releaseToFields: defaults Genre Family to Ambient when not specified', () => {
...
test('safeExternalUrl: passes http(s), rejects javascript: and junk', () => {
  assert.equal(safeExternalUrl('https://zake.bandcamp.com/album/hh'), 'https://zake.bandcamp.com/album/hh');
  assert.equal(safeExternalUrl('  https://room40.bandcamp.com/  '), 'https://room40.bandcamp.com/');
